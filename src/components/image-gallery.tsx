      ...
      <div className="flex space-x-6 min-w-max">
        {images.map((image) => (
          <Link
            key={image.url}
            href={`/style/${image.id}`}
            className="w-[280px] flex-none cursor-pointer transition-transform hover:scale-[1.02]"
          >
            <div className="relative w-full h-[420px]" style={{ background: "transparent", border: "none" }}>
              <Image
